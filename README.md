...
実行時に発生したエラーメッセージを解析し、その根本原因と具体的な解決策を、日本語で、プログラミング知識のない初学者が即座に理解し、次の行動に移せるように要約・解説するシステムです。
...
1. **環境変数の設定**
   `.env.example` をコピーして `.env` を作成し、必要な API キーを入力してください。
...
- Enter キーを押した瞬間に、実行しようとしているコマンドをチェックします。
...
3. **要約**: 解析結果と検索結果をもとに、初心者向けの解説を生成します。
