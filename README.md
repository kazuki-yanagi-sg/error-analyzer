...
`.zshrc` に以下の設定を追加すると、コマンドを実行するたびに自動的にエラー監視が行われるようになります（`w` コマンドを打つ必要もありません）。

```bash
# .zshrc に追記
source /Users/kazukiyanagi/Desktop/Projects/エラー要約システム/src/auto_wrapper.zsh
```

**除外コマンド:**
`cd`, `vim`, `git`, `docker` などの対話的コマンドや特定のコマンドは、自動監視の対象外となります。
